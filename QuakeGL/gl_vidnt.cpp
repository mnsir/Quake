...
int VID_SetMode(int modenum, unsigned char* palette)
{
	...
	if (!msg_suppress_1)
		Con_SafePrintf((char*)"Video mode %s initialized.\n", VID_GetModeDescription(vid_modenum));
...
void VID_SetPalette(unsigned char* palette)
{
	...
	// FIXME: Precalculate this_ and cache to disk.
	for (i = 0; i < (1 << 15); i++)
	{
		/* Maps
		000000000000000
...
BOOL bSetupPixelFormat(HDC hDC)
{
	static PIXELFORMATDESCRIPTOR pfd = {
		sizeof(PIXELFORMATDESCRIPTOR), // size of this_ pfd
		1, // version number
...
void VID_InitDIB(HINSTANCE hInstance)
{
	...
	if (COM_CheckParm((char*)"-width"))
		modelist[0].width = Q_atoi(com_argv[COM_CheckParm((char*)"-width") + 1]);
	else
	...
	if (COM_CheckParm((char*)"-height"))
		modelist[0].height = Q_atoi(com_argv[COM_CheckParm((char*)"-height") + 1]);
	else
...
void VID_Init(unsigned char* palette)
{
	...
	else
	{
		...
		if (COM_CheckParm((char*)"-mode"))
		{
			vid_default = Q_atoi(com_argv[COM_CheckParm((char*)"-mode") + 1]);
		}
		else
		{
			if (COM_CheckParm((char*)"-current"))
			{
				modelist[MODE_FULLSCREEN_DEFAULT].width =
			...
			else
			{
				if (COM_CheckParm((char*)"-width"))
				{
					width = Q_atoi(com_argv[COM_CheckParm((char*)"-width") + 1]);
				}
				else
				{
					width = 640;
				...
				if (COM_CheckParm((char*)"-bpp"))
				{
					bpp = Q_atoi(com_argv[COM_CheckParm((char*)"-bpp") + 1]);
					findbpp = 0;
				}
				else
				{
				...
				if (COM_CheckParm((char*)"-height"))
					height = Q_atoi(com_argv[COM_CheckParm((char*)"-height") + 1]);

				// if they want to force it, add the specified mode to the list
				if (COM_CheckParm((char*)"-force") && (nummodes < MAX_MODE_LIST))
				{
					modelist[nummodes].type = MS_FULLDIB;
	...
	if ((i = COM_CheckParm((char*)"-conwidth")) != 0)
	...
	if ((i = COM_CheckParm((char*)"-conheight")) != 0)
	...
	sprintf(gldir, (char*)"%s/glquake", com_gamedir);
	Sys_mkdir(gldir);
...
void VID_MenuDraw(void)
{
	...
	M_Print(3 * 8, 36 + MODE_AREA_HEIGHT * 8 + 8 * 2, (char*)"Video modes must be set from the");
	M_Print(3 * 8, 36 + MODE_AREA_HEIGHT * 8 + 8 * 3, (char*)"command line with -width <width>");
	M_Print(3 * 8, 36 + MODE_AREA_HEIGHT * 8 + 8 * 4, (char*)"and -bpp <bits-per-pixel>");
	M_Print(3 * 8, 36 + MODE_AREA_HEIGHT * 8 + 8 * 6, (char*)"Select windowed mode with -window");
}
