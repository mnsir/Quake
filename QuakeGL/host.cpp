...
void SV_DropClient(bool crash)
{
    ...
    for (i = 0, client = svs.clients; i < svs.maxclients; i++, client++)
    {
        if (!client->active)
            continue;
        client->message.MSG_WriteByte(svc_updatename);
        client->message.MSG_WriteByte(host_client - svs.clients);
        client->message.MSG_WriteString((char*)"");
        client->message.MSG_WriteByte(svc_updatefrags);
        client->message.MSG_WriteByte(host_client - svs.clients);
        client->message.MSG_WriteShort(0);
        client->message.MSG_WriteByte(svc_updatecolors);
        client->message.MSG_WriteByte(host_client - svs.clients);
