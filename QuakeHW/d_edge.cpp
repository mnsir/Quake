...
void D_DrawPoly()
{
	// this_ driver takes spans, not polygons
}
...
void D_CalcGradients(msurface_t* pface)
{
	...
	TransformVector(ToVec3(pface->texinfo->vecs[0]), p_saxis);
	TransformVector(ToVec3(pface->texinfo->vecs[1]), p_taxis);
...
void D_DrawSurfaces()
{
	...
	// TODO: could preset a lot of this_ at mode set time
	if (r_drawflat.value)
	{
	...
	else
	{
		for (s = &surfaces[1]; s < surface_p; s++)
		{
			...
			else if (s->flags & SURF_DRAWTURB)
			{
				...
				if (s->insubmodel)
				{
					// FIXME: we don't want to do all this_ for every polygon!
					// TODO: store once at start of frame
					currententity = s->entity; //FIXME: make this_ passed in to
					// R_RotateBmodel ()
				...
				if (s->insubmodel)
				{
					...
					// FIXME: we don't want to do this_ every time!
					// TODO: speed up
					//
					currententity = &cl_entities[0];
