...
// FIXME: shouldn't need to include this_
...
void R_EmitEdge(mvertex_t* pv0, mvertex_t* pv1)
{
	...
	else
	{
		...
		VectorSubtract(ToVec3(world), modelorg, local);
	...
	// we need to do this_ to avoid stepping off the edges if a very nearly
	// horizontal edge is less than epsilon above a scan, and numeric error causes
	// it to incorrectly extend to the scan, and the extension of the line goes off
	// the edge of the screen
	// FIXME: is this_ actually needed?
	...
	if (!new_edges[v] || new_edges[v]->u >= u_check)
	{
		edge->next = new_edges[v];
		new_edges[v] = edge;
	}
	else
	{
...
void R_RenderBmodelFace(bedge_t* pedges, msurface_t* psurf)
{
	...
	// this_ is a dummy to give the caching mechanism someplace to write to
	r_pedge = &tedge;
...
void R_RenderPoly(msurface_t* fa, int clipflags)
{
	...
	while (pclip)
	{
		...
		for (i = 0; i < lnumverts; i++)
		{
			...
			if (dist >= 0)
			{
				verts[new_page][new_verts] = verts[vertpage][i];
				new_verts++;
				visible = true;
			}
