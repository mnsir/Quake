...
void R_NewMap()
{
	...
	{
		surfaces = static_cast<surf_t*>(Hunk_AllocName(r_cnumsurfs * sizeof(surf_t), (char*)"surfaces"));
		surface_p = surfaces;
...
void R_ViewChanged(vrect_t* pvrect, int lineadj, float aspect)
{
	...
	r_refdef.horizontalFieldOfView = 2.0 * tan(r_refdef.fov_x / 360 * std::numbers::pi);
	...
	for (int i = 0; i < cl_numvisedicts; i++)
	{
		...
		// trivial accept status
			...
			if (clipflags != BMODEL_FULLY_CLIPPED)
			{
				...
				// at this_ point, so no clipping to the world tree is needed, just
				// frustum clipping
