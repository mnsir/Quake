...
static bool startwindowed = false, windowed_mode_set;
...
cvar_t _windowed_mouse = {(char*)"_windowed_mouse", (char*)"0", true};
cvar_t vid_fullscreen_mode = {(char*)"vid_fullscreen_mode", (char*)"3", true};
cvar_t vid_windowed_mode = {(char*)"vid_windowed_mode", (char*)"0", true};
cvar_t block_switch = {(char*)"block_switch", (char*)"0", true};
cvar_t vid_window_x = {(char*)"vid_window_x", (char*)"0", true};
cvar_t vid_window_y = {(char*)"vid_window_y", (char*)"0", true};
...
void VID_RememberWindowPos(void)
{
	...
	if (GetWindowRect(mainwindow, &rect))
	{
		if ((rect.left < GetSystemMetrics(SM_CXSCREEN)) &&
			...
			(rect.bottom > 0))
		{
			Cvar_SetValue((char*)"vid_window_x", (float)rect.left);
			Cvar_SetValue((char*)"vid_window_y", (float)rect.top);
		}
	}
}
...
void VID_CheckWindowXY(void)
{
	...
	{
		Cvar_SetValue((char*)"vid_window_x", 0.0);
		Cvar_SetValue((char*)"vid_window_y", 0.0);
	}
}
...
bool VID_AllocBuffers(int width, int height)
{
	...
	if ((host_parms.memsize - tbuffersize + SURFCACHE_SIZE_AT_320X200 +
		0x10000 * 3) < minimum_memory)
	{
		Con_SafePrintf((char*)"Not enough memory for video mode\n");
		return false; // not enough memory for mode
	}
...
int VID_Suspend(MGLDC* dc, m_int flags)
{
	...
	{
		// FIXME: this_ doesn't currently work on NT
...
void VID_InitMGLFull(HINSTANCE hInstance)
{
	...
	if (COM_CheckParm((char*)"-nowindirect") ||
		COM_CheckParm((char*)"-nowd") ||
		COM_CheckParm((char*)"-novesa") ||
		WinNT)
	{
		useWinDirect = false;
	}

	if (COM_CheckParm((char*)"-nodirectdraw") || COM_CheckParm((char*)"-noddraw") || COM_CheckParm((char*)"-nodd"))
		useDirectDraw = false;
	...
	if (m[0] != 0xFF)
	{
		...
		for (i = 0; m[i] != 0xFF; i++)
		{
				...
				(curmode < MAX_MODE_LIST))
			{
				...
				if (!COM_CheckParm((char*)"-noforcevga"))
				{
					if (m[i] == grVGA_320x200x256)
					{
						mode = i;
		...
		for (i = 0; m[i] != 0xFF; i++)
		{
				...
				(nummodes < MAX_MODE_LIST))
			{
				...
				sprintf(modelist[curmode].modedesc, (char*)"%dx%d", xRes, yRes);
		...
		if (lowstretched)
		{
			...
			sprintf(modelist[MODE_FULLSCREEN_DEFAULT].modedesc, (char*)"%dx%d",
...
****************************************************************************/
{
	...
	if (!COM_CheckParm((char*)"-notriplebuf"))
	{
		if (npages > 2)
		{
			npages = 2;
...
void VID_InitFullDIB(HINSTANCE hInstance)
{
	...
	// see if any of them were actually settable; if so, this_ is our mode list,
	...
	if (nummodes == originalnummodes)
	{
		...
		Con_SafePrintf((char*)"No 8-bpp fullscreen DIB modes found\n");
		...
		do
		{
			...
			if ((((devmode.dmPelsWidth <= MAXWIDTH) &&
						(devmode.dmPelsHeight <= MAXHEIGHT)) ||
					(!COM_CheckParm((char*)"-noadjustaspect") &&
	...
	if (!is_mode0x13)
	{
		...
		if (cstretch > 0)
		{
			...
			sprintf(modelist[istretch].modedesc, (char*)"%dx%d",
...
char* VID_GetModeDescription2(int mode)
{
	...
	{
		sprintf(pinfo, (char*)"%s fullscreen", pv->modedesc);
	}
	...
	{
		sprintf(pinfo, (char*)"%s windowed", pv->modedesc);
	}
...
// KJB: Added this_ to return the mode driver name in description for console
...
char* VID_GetExtModeDescription(int mode)
{
	...
	{
		sprintf(pinfo, (char*)"%s fullscreen DIB", pv->modedesc);
	}
	else
	{
		sprintf(pinfo, (char*)"%s windowed", pv->modedesc);
...
bool VID_SetWindowedMode(int modenum)
{
	...
	{
		if (COM_CheckParm((char*)"-resetwinpos"))
		{
			Cvar_SetValue((char*)"vid_window_x", 0.0);
			Cvar_SetValue((char*)"vid_window_y", 0.0);
		}
	...
	SendMessage(mainwindow, WM_SETICON, TRUE, (LPARAM)hIcon);
	SendMessage(mainwindow, WM_SETICON, FALSE, (LPARAM)hIcon);
...
bool VID_SetFullscreenMode(int modenum)
{
	...
	SendMessage(mainwindow, WM_SETICON, 1, (LPARAM)hIcon);
	SendMessage(mainwindow, WM_SETICON, 0, (LPARAM)hIcon);

	// shouldn't be needed, but Kendall needs to let us get the activation
	// message for this_ not to be needed on NT
...
int VID_SetMode(int modenum, unsigned char* palette)
{
	...
	while ((modenum >= nummodes) || (modenum < 0))
	{
		if (vid_modenum == NO_MODE)
		{
			...
			Cvar_SetValue((char*)"vid_mode", (float)modenum);
		}
		else
		{
			Cvar_SetValue((char*)"vid_mode", (float)vid_modenum);
			return 0;
		}
	}
	...
	{
		// couldn't get memory for this_ mode; try to fall back to previous mode
		VID_RestoreOldMode(original_mode);
		return false;
	}
...
void VID_Init(unsigned char* palette)
{
	...
	Cmd_AddCommand((char*)"vid_testmode", VID_TestMode_f);
	Cmd_AddCommand((char*)"vid_nummodes", VID_NumModes_f);
	Cmd_AddCommand((char*)"vid_describecurrentmode", VID_DescribeCurrentMode_f);
	Cmd_AddCommand((char*)"vid_describemode", VID_DescribeMode_f);
	Cmd_AddCommand((char*)"vid_describemodes", VID_DescribeModes_f);
	Cmd_AddCommand((char*)"vid_forcemode", VID_ForceMode_f);
	Cmd_AddCommand((char*)"vid_windowed", VID_Windowed_f);
	Cmd_AddCommand((char*)"vid_fullscreen", VID_Fullscreen_f);
	Cmd_AddCommand((char*)"vid_minimize", VID_Minimize_f);

	if (COM_CheckParm((char*)"-dibonly"))
		dibonly = true;
	...
	if (COM_CheckParm((char*)"-startwindowed"))
	{
		startwindowed = true;
...
void VID_Shutdown(void)
{
	...
	if (vid_initialized)
	{
		...
		PostMessage(HWND_BROADCAST, WM_PALETTECHANGED, (WPARAM)mainwindow, 0);
		PostMessage(HWND_BROADCAST, WM_SYSCOLORCHANGE, 0, 0);
...
void VID_Update(vrect_t* rects)
{
	...
	if (firstupdate)
	{
			...
			(!startwindowed || (_vid_default_mode_win.value < MODE_FULLSCREEN_DEFAULT)))
		{
			...
			{
				Cvar_SetValue((char*)"_vid_default_mode_win", windowed_default);
			}

			Cvar_SetValue((char*)"vid_mode", _vid_default_mode_win.value);
		}
	}
		...
		if (!ActiveApp)
		{
			if (modestate == MS_FULLDIB)
			{
				if (vid_initialized)
				{
					...
					// we never seem to get WM_ACTIVATE inactive from this_ mode set, so we'll
					// do it manually
					t = in_mode_set;
					in_mode_set = true;
	...
	LPARAM lParam)
{
	...
	case WM_MOUSEWHEEL:
		...
		break;
	// KJB: Added these new_ palette functions
	...
	case WM_QUERYNEWPALETTE:
		...
		if (vid_initialized && !in_mode_set && windc && MGL_activatePalette(windc, false) && !Minimized)
		{
			...
			// specifically required if WM_QUERYNEWPALETTE realizes a new_ palette
			lRet = TRUE;
		}
		break;
	...
	case WM_CLOSE:
		// this_ causes Close in the right-click task bar menu not to work, but right
...
void VID_MenuDraw(void)
{
	...
	for (i = 3; i < lnummodes; i++)
	{
		...
		if (ptr && ((pv->width != 360) || COM_CheckParm((char*)"-allow360")))
		{
			...
			{
				if (!dup || !modedescs[dupmode].ismode13 || COM_CheckParm((char*)"-noforcevga"))
				{
					if (dup)
					{
						k = dupmode;
	...
	if (vid_wmodes > 3)
	{
		M_Print(12 * 8, 36 + 4 * 8, (char*)"Fullscreen Modes");
	...
	else
	{
		...
		if (ptr)
		{
			sprintf(temp, (char*)"Current default: %s", ptr);
...
void VID_MenuKey(int key)
{
	...
	case 't':
		S_LocalSound((char*)"misc/menu1.wav");
	// have to set this_ before setting the mode because WM_PAINT
	...
	case 'd':
		S_LocalSound((char*)"misc/menu1.wav");
		firstupdate = 0;
		Cvar_SetValue((char*)"_vid_default_mode_win", vid_modenum);
		break;
