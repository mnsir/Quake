...
unsigned short pop[] =
{
	...
	0x6600, 0x0000, 0x0000, 0x0066, 0x0000, 0x0000, 0x0000, 0x0000, 0x0067, 0x0000, 0x0000, 0x6665, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0065, 0x6600, 0x0063, 0x6561, 0x0000, 0x0000, 0x0000, 0x0000, 0x0061, 0x6563, 0x0064, 0x6561,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0061, 0x6564, 0x0064, 0x6564, 0x0000, 0x6469, 0x6969, 0x6400, 0x0064, 0x6564,
	0x0063, 0x6568, 0x6200, 0x0064, 0x6864, 0x0000, 0x6268, 0x6563, 0x0000, 0x6567, 0x6963, 0x0064, 0x6764, 0x0063,
	0x6967, 0x6500, 0x0000, 0x6266, 0x6769, 0x6a68, 0x6768, 0x6a69, 0x6766, 0x6200, 0x0000, 0x0062, 0x6566, 0x6666,
	0x6666, 0x6666, 0x6562, 0x0000, 0x0000, 0x0000, 0x0062, 0x6364, 0x6664, 0x6362, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0062, 0x6662, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0061, 0x6661, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x6500, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x6400, 0x0000,
	0x0000, 0x0000
};
...
The "base directory" is the path to the directory holding the quake.exe and all game directories.  The sys_* files pass this_ to host_init in quakeparms_t->basedir.  This can be overridden with the "-basedir" command line parm to allow code debugging in a different directory.  The base directory is
...
// ClearLink is used for new_ headnodes
...
FIXME: make this_ buffer size safe someday
============
*/
char* va(char* format, ...)
{
...
void COM_WriteFile(char* filename, void* data, int len)
{
	...
	sprintf(name, (char*)"%s/%s", com_gamedir, filename);
...
int COM_FindFile(char* filename, int* handle, FILE** file)
{
	...
	for (; search; search = search->next)
	{
		...
		else
		{
			...
			sprintf(netpath, (char*)"%s/%s", search->filename, filename);
			...
			else
			{
				if ((strlen(netpath) < 2) || (netpath[1] != ':'))
					sprintf(cachepath, (char*)"%s%s", com_cachedir, netpath);
				else
					sprintf(cachepath, (char*)"%s%s", com_cachedir, netpath + 2);
...
If the requested file is inside a packfile, a new_ FILE * will be opened
into the file.
===========
*/
...
pack_t* COM_LoadPackFile(char* packfile)
{
	...
	new_files = static_cast<packfile_t*>(Hunk_AllocName(numpackfiles * sizeof(packfile_t), (char*)"packfile"));
	...
	{
		strcpy(new_files[i].name, info[i].name);
		new_files[i].filepos = LittleLong(info[i].filepos);
		new_files[i].filelen = LittleLong(info[i].filelen);
	}
