...
// this_ means that advanced controller configuration needs to be executed
// each time.  this_ avoids any problems with getting back to a default usage
// or when changing from one controller to another.  this_ way at least something
// works.
...
static DIDATAFORMAT df = {
	sizeof(DIDATAFORMAT), // this_ structure
...
void IN_ActivateMouse(void)
{
	...
	if (mouseinitialized)
	{
		...
		{
			if (mouseparmsvalid)
				restore_spi = SystemParametersInfo(SPI_SETMOUSE, 0, new_mouseparms, 0);
	...
	if (!dinput)
	{
		...
		if (mouseparmsvalid)
		{
			if (COM_CheckParm((char*)"-noforcemspd"))
				new_mouseparms[2] = originalmouseparms[2];

			if (COM_CheckParm((char*)"-noforcemaccel"))
			{
				new_mouseparms[0] = originalmouseparms[0];
				new_mouseparms[1] = originalmouseparms[1];
			}
...
void IN_StartupJoystick(void)
{
	...
	// this_ is needed as cvars are not available during initialization
...
void Joy_AdvancedUpdate_f(void)
{
	...
	{
		if (Q_strcmp(joy_name.string, (char*)"joystick") != 0)
		{
...
bool IN_ReadJoystick(void)
{
	...
	{
		// this_ is a hack -- there is a bug in the Logitech WingMan Warrior DirectInput Driver
	...
	// Con_Printf ((char*)"IN_ReadJoystick: no response\n");
	// joy_avail = false;
	return false;
}
...
void IN_JoyMove(usercmd_t* cmd)
{
	...
	for (int i = 0; i < JOY_MAX_AXES; i++)
	{
		...
		if (joy_wwhack2.value != 0.0)
		{
			...
			{
				// this_ is a special formula for the Logitech WingMan Warrior
		...
		case AxisForward:
			if ((joy_advanced.value == 0.0) && (in_mlook.state & 1))
			{
				...
				else
				{
					...
					// *** this_ code can be removed when the lookspring bug is fixed
