...
void NET_Ban_f(void)
{
	...
	case 2:
		if (Q_strcasecmp(Cmd_Argv(1), (char*)"off") == 0)
			banAddr = 0x00000000;
		else
...
void NET_Stats_f(void)
{
	...
	else if (Q_strcmp(Cmd_Argv(1), (char*)"*") == 0)
	{
...
static qsocket_t* _Datagram_CheckNewConnections(void)
{
	...
	{
		if (Q_strcmp(MSG_ReadString(), (char*)"QUAKE") != 0)
			return NULL;
		...
		dfunc.GetSocketAddr(acceptsock, &new_addr);
		MSG_WriteString(&net_message, dfunc.AddrToString(&new_addr));
	...
	{
		unsigned long testAddr = ((struct sockaddr_in*)&clientaddr)->sin_addr.s_addr;
	...
	// see if this_ guy is already connected
	for (qsocket_t* s = net_activeSockets; s; s = s->next)
	{
		...
		if (ret >= 0)
		{
			// is this_ a duplicate connection reqeust?
			if (ret == 0 && net_time - s->connecttime < 2.0)
			{
				...
				dfunc.GetSocketAddr(s->socket, &new_addr);
				MSG_WriteLong(&net_message, dfunc.GetSocketPort(&new_addr));
				*((int*)net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
				dfunc.Write(acceptsock, net_message.data, net_message.cursize, &clientaddr);
				SZ_Clear(&net_message);
				return NULL;
	...
	int new_sock = dfunc.OpenSocket(0);
	if (new_sock == -1)
	{
	...
	if (dfunc.Connect(new_sock, &clientaddr) == -1)
	{
		dfunc.CloseSocket(new_sock);
		NET_FreeQSocket(sock);
		return NULL;
	}

	// everything is allocated, just fill in the details 
	sock->socket = new_sock;
	...
	MSG_WriteLong(&net_message, 0);
	MSG_WriteByte(&net_message, CCREP_ACCEPT);
	dfunc.GetSocketAddr(new_sock, &new_addr);
	MSG_WriteLong(&net_message, dfunc.GetSocketPort(&new_addr));
	// MSG_WriteString(&net_message, dfunc.AddrToString(&new_addr));
	*((int*)net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
	dfunc.Write(acceptsock, net_message.data, net_message.cursize, &clientaddr);
...
static void _Datagram_SearchForHosts(bool xmit)
{
	...
	while ((ret = dfunc.Read(dfunc.controlSock, net_message.data, net_message.maxsize, &readaddr)) > 0)
	{
		...
		// search the cache for this_ server
		for (n = 0; n < hostCacheCount; n++)
		...
		if (MSG_ReadByte() != NET_PROTOCOL_VERSION)
		{
			...
			Q_strcpy(hostcache[n].name, (char*)"*");
...
static qsocket_t* _Datagram_Connect(char* host)
{
	...
	// connect to the host
	if (dfunc.Connect(new_sock, &sendaddr) == -1)
		goto ErrorReturn;
	...
	for (int reps = 0; reps < 3; reps++)
	{
		...
		MSG_WriteByte(&net_message, NET_PROTOCOL_VERSION);
		*((int*)net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
		dfunc.Write(new_sock, net_message.data, net_message.cursize, &sendaddr);
		SZ_Clear(&net_message);
		do
		{
			ret = dfunc.Read(new_sock, net_message.data, net_message.maxsize, &readaddr);
	...
	if (dfunc.Connect(new_sock, &sock->addr) == -1)
	{
