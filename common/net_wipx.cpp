...
int WIPX_OpenSocket(int port)
{
	...
	if ((new_socket = psocket(AF_IPX, SOCK_DGRAM, NSPROTO_IPX)) == INVALID_SOCKET)
		return -1;

	if (pioctlsocket(new_socket, FIONBIO, &_true) == -1)
		goto ErrorReturn;

	if (psetsockopt(new_socket, SOL_SOCKET, SO_BROADCAST, (char*)&_true, sizeof(_true)) < 0)
		goto ErrorReturn;
...
char* WIPX_AddrToString(struct qsockaddr* addr)
{
	...
	sprintf(buf, (char*)"%02x%02x%02x%02x:%02x%02x%02x%02x%02x%02x:%u",
...
int WIPX_GetAddrFromName(char* name, struct qsockaddr* addr)
{
	...
	if (n == 12)
	{
		sprintf(buf, (char*)"00000000:%s:%u", name, net_hostport);
		return WIPX_StringToAddr(buf, addr);
	}
	if (n == 21)
	{
		sprintf(buf, (char*)"%s:%u", name, net_hostport);
		return WIPX_StringToAddr(buf, addr);
	}
	if (n > 21 && n <= 27)
