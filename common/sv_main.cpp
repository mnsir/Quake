...
Initializes a client_t for a new_ net connection.  This will only be called
...
void SV_SpawnServer(char* server)
{
	...
	//
	// tell all connected clients that we are going to a new_ level
	//
	if (sv.active)
	{
		SV_SendReconnect();
	...
	sv.edicts = static_cast<edict_t*>(Hunk_AllocName(sv.max_edicts * pr_edict_size, (char*)"edicts"));
	...
	strcpy(sv.name, server);
	sprintf(sv.modelname, (char*)"maps/%s.bsp", server);
