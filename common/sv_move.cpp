...
qboolean SV_movestep(edict_t* ent, vec3_t move, qboolean relink)
{
	...
	if ((int)ent->v.flags & (FL_SWIM | FL_FLY))
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			trace = SV_Move(ent->v.origin, ent->v.mins, ent->v.maxs, new_org, false, ent);
	...
	trace = SV_Move(new_org, ent->v.mins, ent->v.maxs, end, false, ent);
	...
	if ((int)ent->v.flags & FL_PARTIALGROUND)
	{
		// Con_Printf ((char*)"back on ground\n"); 
...
{
	// Con_Printf ((char*)"SV_FixCheckBottom\n");
