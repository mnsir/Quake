...
int SV_FlyMove(edict_t* ent, float time, trace_t* steptrace)
{
	...
	for (int bumpcount = 0; bumpcount < numbumps; bumpcount++)
	{
		...
		{
			ClipVelocity(original_velocity, planes[i], new__velocity, 1);
			for (j = 0; j < numplanes; j++)
				if (j != i)
				{
					if (DotProduct(new__velocity, planes[j]) < 0)
						break; // not ok
				}
			if (j == numplanes)
				break;
		...
		if (i != numplanes)
		{
			// go along this_ plane
			VectorCopy(new__velocity, ent->v.velocity);
		}
		else
		{
			// go along the crease
			if (numplanes != 2)
			{
				// Con_Printf ((char*)"clip velocity, numplanes == %i\n",numplanes);
				VectorCopy(vec3_origin, ent->v.velocity);
				return 7;
			}
...
void SV_AddGravity(edict_t* ent)
{
	...
	eval_t* val = GetEdictFieldValue(ent, (char*)"gravity");
	if (val && val->_float)
