...
float V_CalcBob()
{
	...
	if (cycle < cl_bobup.value)
		cycle = std::numbers::pi * cycle / cl_bobup.value;
	else
		cycle = std::numbers::pi + std::numbers::pi * (cycle - cl_bobup.value) / (1.0 - cl_bobup.value);

	// bob is proportional to velocity in the xy plane
	// (don't count Z, or jumping messes it up)

	float bob = sqrt(cl.velocity[0] * cl.velocity[0] + cl.velocity[1] * cl.velocity[1]) * cl_bob.value;
	//Con_Printf ((char*)"speed: %5.1f\n", Length(cl.velocity));
...
void V_UpdatePalette()
{
	...
	for (i = 0; i < 256; i++)
	{
		...
		new_pal[0] = ramps[0][ir];
		new_pal[1] = ramps[1][ig];
		new_pal[2] = ramps[2][ib];
		new_pal += 3;
	}
...
void V_UpdatePalette()
{
	...
	for (i = 0; i < 256; i++)
	{
		...
		new_pal[0] = gammatable[r];
		new_pal[1] = gammatable[g];
		new_pal[2] = gammatable[b];
		new_pal += 3;
	}
