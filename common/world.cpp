...
hull_t* SV_HullForEntity(edict_t* ent, const vec3_t& mins, const vec3_t& maxs, vec3_t& offset)
{
	...
	if (ent->v.solid == SOLID_BSP)
	{
		...
		if (!model || model->GetModType() != modtype_t::mod_brush)
...
void SV_ClearWorld()
{
	...
	sv_numareanodes = 0;
	SV_CreateAreaNode(0, sv.worldmodel->GetMins(), sv.worldmodel->GetMaxs());
}
...
void SV_LinkEdict(edict_t* ent, bool touch_triggers)
{
	...
	// if touch_triggers, touch all entities at this_ node and decend for more
	if (touch_triggers)
...
bool SV_RecursiveHullCheck(hull_t* hull, int num, float p1f, float p2f, const vec3_t& p1, const vec3_t& p2, trace_t* trace)
{
	...
	//==================
	// the other side of the node is solid, this_ is the impact point
	//==================
	if (!side)
	{
